...
def weight_of_order(order, world):
    weight = 0
    for item in order.items:
        weight += weight_of_item(item, world)
    return weight

def sorted_orders_lightest_first(orders, world):
    return sorted(orders, key = lambda o: weight_of_order(o, world))
...
def prepare_routes(world, state):
    ...
    # TODO TODO TODO TODO TODO TODO
    # TODO TODO TODO TODO TODO TODO
    # Build routes from the warehouses.
    #
    # Either
    # 1. start with outmost warehouses and redistribute stuff from there first
    #    (this would mean that stuff would end in the center warehouses, which
    #    does not seem really efficient)
    # or
    # 2. start with center warehouses and fill orders from there and then ?

    temp_warehouses = copy.deepcopy(state.warehouses)

    # for each order find the nearest warehouses from which it can be fulfilled
    for o in sorted_orders_lightest_first(state.open_orders, world):
        ...
        for r in o_routes:
            r.routes_per_order = len(o_routes)
